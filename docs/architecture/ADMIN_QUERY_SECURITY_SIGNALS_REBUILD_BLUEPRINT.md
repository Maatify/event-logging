...
No known package-internal references exist. This is a public library; the superseded wrapper has not been fully adopted by any host project. However, every maintained host repository MUST be searched before deletion. Any discovered usage must be migrated to the replacement Admin Query API before the exact seven superseded Runtime/test artifacts can be removed. This is a strict verification/migration gate. Deletion is NOT authorized in PR #102.
...
* `actorType` is valid without `actorId`.
* Both may be supplied together. Note that querying `actorId` without `actorType` will bypass the leftmost prefix of `idx_el_security_signals_actor_time`, which is an accepted performance implication rather than a validation restriction.
...
Primitive queries repeat `:cursor_at`. Distinct placeholders are a required project standard. A behavior-preserving correction is strictly required for the future Runtime implementation:
```sql
...
```
* Future Runtime must use `:cursor_at_before` and `:cursor_at_equal` with the same UTC-formatted value.
* Must preserve primitive signature, cursor activation, ordering, limit, and results.
* NOT authorized in PR #102.
* Requires strict coverage with real-MySQL native-prepared-statement tests.

## N. Complete Test and Verification Plan

**Unit Matrix:**
* DTO constructor/serialization, trim/empty logic, UTF-8 validity, length bounds, positive actor ID, independent actor pair combinations, date range, sort normalization.
...
* Deterministic ordering, limit behavior, and cursor results remain identical with distinct PDO placeholders.
...
* Single and combined filters (including `actorId` alone), inclusive/equal date bounds, zero results, multiple pages, clamping.
