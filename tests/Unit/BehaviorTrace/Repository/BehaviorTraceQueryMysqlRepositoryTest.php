...
class BehaviorTraceQueryMysqlRepositoryTest extends TestCase
{
    public function testFindWithNoFiltersReturnsDefaultQuery(): void
    {
        ...
        $query = new BehaviorTraceQueryDTO(limit: 50);
        $repository->find($query);

        $this->assertNotNull($pdo->lastStatement);
        $this->assertEquals(
            'SELECT * FROM maa_event_logging_behavior_trace  ORDER BY occurred_at DESC, id DESC LIMIT 50',
    ...
    public function testFindWithFiltersConstructsCorrectWhereClause(): void
    {
        ...
        $this->assertNotNull($pdo->lastStatement);
        $this->assertStringContainsString('WHERE actor_type = :actor_type AND actor_id = :actor_id AND entity_type = :entity_type AND entity_id = :entity_id AND action = :action AND request_id = :request_id AND correlation_id = :correlation_id AND occurred_at >= :after AND occurred_at <= :before', $pdo->lastStatement->queryString);
    ...
    public function testReadLegacyMethodYieldsCorrectly(): void
    {
        ...
        // Simulate fetching two rows then false
        ...
        $repository = new BehaviorTraceQueryMysqlRepository($mockPdo);
        $cursor = new BehaviorTraceCursorDTO(new DateTimeImmutable('2024-01-01 11:00:00', new DateTimeZone('UTC')), 0);
    ...
    public function testFindThrowsStorageExceptionOnPdoFailure(): void
    {
        ...
        $this->expectExceptionMessage('Failed to query BehaviorTrace records');
